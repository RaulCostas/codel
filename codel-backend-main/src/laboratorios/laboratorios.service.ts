...
export class LaboratoriosService {
    ...
    async create(createLaboratorioDto: CreateLaboratorioDto) {
        const existing = await this.laboratoriosRepository.findOne({
            where: { laboratorio: ILike(createLaboratorioDto.laboratorio.trim()) }
        });

        if (existing) {
            throw new BadRequestException('Este laboratorio ya se encuentra registrado.');
        }
    ...
    async update(id: number, updateLaboratorioDto: UpdateLaboratorioDto) {
        if (updateLaboratorioDto.laboratorio) {
            const existing = await this.laboratoriosRepository.createQueryBuilder('lab')
                .where('LOWER(lab.laboratorio) = LOWER(:nombre)', { nombre: updateLaboratorioDto.laboratorio.trim() })
                .andWhere('lab.id != :id', { id })
                .getOne();

            if (existing) {
                throw new BadRequestException('Ya existe otro laboratorio con este nombre.');
            }
        }
