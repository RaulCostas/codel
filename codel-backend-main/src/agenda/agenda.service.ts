...
export class AgendaService {
    ...
    async enviarRecordatoriosManana(fecha?: string): Promise<{ success: boolean; message: string; enviados: number; fallidos: number }> {
        ...
        for (const cita of citasParaRecordar) {
            let p: any = null;
            try {
                p = cita.paciente || cita.pacienteSeguro;
                if (!p) continue;
    ...
    async findAll(date?: string, fechaInicio?: string, fechaFinal?: string, pacienteId?: number, usuarioId?: number, pacienteSeguroId?: number): Promise<Agenda[]> {
        const query = this.agendaRepository.createQueryBuilder('agenda')
            .leftJoinAndSelect('agenda.paciente', 'paciente')
            .leftJoinAndSelect('agenda.pacienteSeguro', 'pacienteSeguro')
            .leftJoinAndSelect('pacienteSeguro.seguro', 'seguroRel')
        ...
        // Filter by insurance patient
        if (pacienteSeguroId) {
            query.andWhere('agenda.pacienteSeguroId = :pacienteSeguroId', { pacienteSeguroId });
        }
    ...
    async findAllByPacienteSeguro(pacienteSeguroId: number): Promise<Agenda[]> {
        return await this.agendaRepository.find({
            where: { pacienteSeguroId } as any,
            relations: {
                pacienteSeguro: {
                    seguro: true
                },
    ...
    async update(id: number, updateDto: UpdateAgendaDto): Promise<Agenda> {
        ...
        if (updateDto.pacienteSeguroId !== undefined && updateDto.pacienteSeguroId !== cita.pacienteSeguroId) {
            console.log(`[AgendaService] Insurance Patient change detected: ${cita.pacienteSeguroId} -> ${updateDto.pacienteSeguroId}. Clearing relation object.`);
            (cita as any).pacienteSeguro = null;
        }
