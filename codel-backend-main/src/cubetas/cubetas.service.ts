...
export class CubetasService {
    ...
    async create(createDto: CreateCubetaDto): Promise<Cubeta> {
        const { descripcion } = createDto;
        
        const existing = await this.cubetaRepository.findOne({
            where: { descripcion: ILike(descripcion.trim()) }
        });

        if (existing) {
            throw new BadRequestException(`Ya existe una cubeta con la descripción "${descripcion}".`);
        }
    ...
    async findAll(page: number = 1, limit: number = 10, search: string = '', dentro_fuera: string = ''): Promise<{ data: Cubeta[], total: number, page: number, limit: number, totalPages: number }> {
    ...
    async update(id: number, updateDto: UpdateCubetaDto): Promise<Cubeta> {
        if (updateDto.descripcion) {
            const { descripcion } = updateDto;
            const existing = await this.cubetaRepository.findOne({
                where: { 
                    descripcion: ILike(descripcion.trim()),
                    id: Not(id)
                }
            });

            if (existing) {
                throw new BadRequestException(`Ya existe otra cubeta con la descripción "${descripcion}".`);
            }
        }
