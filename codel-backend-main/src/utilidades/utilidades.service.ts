...
export class UtilidadesService {
    constructor(
        ...
        @InjectRepository(ProformaSeguro) private proformaSeguroRepo: Repository<ProformaSeguro>,
    ) { }
    ...
    async getStatistics(year: string) {
        ...
        try {
            ...
            // 1.1 INGRESOS (Seguros)
            const proformasSeguro = await this.proformaSeguroRepo.createQueryBuilder('ps')
                .where('ps.estado = :estado', { estado: 'pagada' })
                .andWhere('EXTRACT(YEAR FROM ps.fecha_pago::date) = :year', { year })
                .getMany();
            
            proformasSeguro.forEach(ps => {
                updateStat(getMonth(ps.fecha_pago), 'Bolivianos', 'ingresos', Number(ps.total));
            });
