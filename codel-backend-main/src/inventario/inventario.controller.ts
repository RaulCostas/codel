...
export class InventarioController {
    ...
    @Get('used-groups')
    findUsedGroups() {
        return this.inventarioService.findUsedGroups();
    }

    @Get('used-specialties')
    findUsedSpecialties(@Query('grupoId') grupoId?: string) {
        return this.inventarioService.findUsedSpecialties(
            grupoId && grupoId !== 'all' ? Number(grupoId) : undefined
        );
    }
    ...
    findAll(
        ...
        return this.inventarioService.findAll(search, page ? +page : 1, limit ? +limit : 10);
    }
