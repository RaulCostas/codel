...
export class ProformasService {
  ...
  async uploadPacienteImage(pacienteId: number, tipo: 'particular' | 'seguro', filename: string, buffer: Buffer, mimetype: string, descripcion?: string) {
    // Upload to Supabase
    const bucket = 'clinica-media';
    const folder = tipo === 'particular' ? 'pacientes' : 'pacientes_seguro';
    ...
    if (tipo === 'particular') {
      ...
      imagen.pacienteSeguroId = pacienteId;
    }
    imagen.nombre_archivo = filename;
    imagen.ruta = publicUrl;
    imagen.descripcion = descripcion || '';
  ...
  async getPacienteImages(pacienteId: number, tipo: 'particular' | 'seguro') {
    const where: any = {};
    if (tipo === 'particular') {
      where.pacienteId = pacienteId;
    } else {
      where.pacienteSeguroId = pacienteId;
    }
    return this.imagenRepository.find({
      where,
      order: { fecha_creacion: 'DESC' }
