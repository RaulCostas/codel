    ...
    async getStatistics(fechaInicio: string, fechaFinal: string, status?: string): Promise<any[]> {
        ...
        const qb = this.doctorsRepository.createQueryBuilder('doctor')
            ...
            .select([
                ...
                // Calculate total as price reduced by the proportional discount of the proforma
                // Formula: precio * (total_proforma / sub_total_proforma)
                `COALESCE(SUM(hc.precio * (CASE WHEN COALESCE(p.sub_total, 0) > 0 THEN (p.total / p.sub_total) ELSE 1 END)), 0) AS "totalGenerado"`
            ])
            .groupBy('doctor.id')
