...
const UnidadMedidaList: React.FC = () => {
    ...
    const manualSections: ManualSection[] = [
        {
            ...
            content: 'Aquí puede administrar las unidades de medida del inventario (ej. Caja, Blíster, Unidad, Frasco, Tubo). Use "+ Nueva Unidad" para registrar una nueva opción.'
        },
        {
            title: 'Dar de Baja y Reactivar',
            content: 'Al dar de baja una unidad de medida, pasará a estado "Inactivo". Los registros existentes del inventario conservarán su unidad de medida anterior, pero ya no se podrá seleccionar esta unidad para nuevos productos.'
        }
    ];
    ...
    const fetchUnidades = async () => {
        try {
            ...
            const response = await api.get<PaginatedResponse>(`/unidad-medida?${params}`);
    ...
    const handleDelete = async (id: number) => {
        const result = await Swal.fire({
            title: '¿Dar de baja unidad de medida?',
            text: 'La unidad pasará a estado inactivo y no se mostrará para nuevos productos.',
            icon: 'warning',
        ...
        if (result.isConfirmed) {
            try {
                ...
                await Swal.fire({ icon: 'success', title: '¡Unidad dada de baja!', showConfirmButton: false, timer: 1500 });
                fetchUnidades();
            } catch (error) {
                console.error('Error:', error);
                Swal.fire('Error', 'No se pudo dar de baja la unidad', 'error');
            }
        }
    };
    ...
    const handleReactivate = async (id: number) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                await Swal.fire({ icon: 'success', title: '¡Unidad reactivada!', showConfirmButton: false, timer: 1500 });
                fetchUnidades();
            } catch (error) {
                console.error('Error:', error);
                Swal.fire('Error', 'No se pudo reactivar la unidad', 'error');
            }
        }
    };
    ...
    const handlePrint = async () => {
        try {
            const response = await api.get<PaginatedResponse>(`/unidad-medida?page=1&limit=9999${searchTerm ? `&search=${searchTerm}` : ''}`);
            const allUnidades = response.data?.data || [];
            ...
            const printContent = `
                ...
                <body>
                    ...
                    <table>
                        ...
                        <tbody>
                            ${allUnidades.map((u: UnidadMedida, index: number) => `
                                <tr>
                                    <td>${index + 1}</td>
                                    <td>${u.nombre || '-'}</td>
                                    <td class="${u.estado === 'activo' ? 'status-active' : 'status-inactive'}">
                                        ${u.estado ? (u.estado.charAt(0).toUpperCase() + u.estado.slice(1)) : 'Inactivo'}
                                    </td>
                                </tr>
                            `).join('')}
                        </tbody>
        ...
        <div className="content-card p-6 bg-gray-50 dark:bg-gray-800 min-h-screen">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 no-print gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col">
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Gestión de unidades para productos del inventario</p>
                    </div>
                </div>
                        ...
                        {unidades.map((unidad, index) => (
                            <tr key={unidad.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                    <span className={`px-2 py-1 rounded text-sm ${unidad.estado === 'activo'
            ...
            {unidades.length === 0 && (
                <div className="p-4 text-center text-gray-500">
                    {searchTerm ? 'No se encontraron resultados' : 'No hay unidades de medida registradas.'}
                </div>
            )}
