...
const HojaDiaria: React.FC = () => {
    ...
    const [ingresosSeguros, setIngresosSeguros] = useState<any[]>([]);
    ...
    const calculateAllTotals = (
        ...
        ings.forEach(item => {
            ...
            if (currency.toUpperCase().includes('BOLIVIANO') || currency.toUpperCase() === 'BS') { 
                ingresosBs += amount; 
        ...
        const addEgreso = (items: any[], type: string) => {
            items.forEach(item => {
                ...
                const currency = item.moneda || 'Bolivianos';
                const paymentMethod = (type === 'pedido' ? item.forma_pago : item.formaPago?.forma_pago) || 'N/A';
                const entry = getSummaryEntry(paymentMethod);
    ...
    const fetchPreviousData = async (modeOverride?: 'single' | 'range') => {
        try {
            ...
            const [resIngresos, resEgresos, resDoctores, resLaboratorios, resPedidos, resGastosFijos, resSeguros] = await Promise.all([
                ...
                api.get('/proforma-seguro', { params: prevParams })
            ]);
            const totals = calculateAllTotals(
                resIngresos.data, resEgresos.data.data || [], resDoctores.data,
                resLaboratorios.data, resPedidos.data, resGastosFijos.data, resSeguros.data
            );
    ...
    const fetchAllData = async (modeOverride?: 'single' | 'range') => {
        ...
        try {
            ...
            const [
                ...
                api.get('/proforma-seguro', { params })
            ]);
            ...
            setIngresosSeguros(resSeguros.data);

            const totals = calculateAllTotals(
                resIngresos.data,
                resEgresos.data.data || [],
                resDoctores.data,
                resLaboratorios.data,
                resPedidos.data,
                resGastosFijos.data,
                resSeguros.data
            );
    ...
    const handlePrintSeguros = () => {
        ...
        const printContent = `
            ...
            <html>
            <head>
                <title>Pagos de Seguros - Hoja Diaria</title>
                <style>
                    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap');
                    @page { size: A4 landscape; margin: 1.5cm; }
                    body { font-family: 'Inter', sans-serif; margin: 0; padding: 0; color: #1e293b; background: white; line-height: 1.5; }
                    ...
                    .amount { font-weight: 700; color: #059669; text-align: right; }
            ...
            <body>
                <div class="header">
                    <div class="title-container">
                        <img src="${logoUrl}" alt="Logo">
                        <h1>Pagos de Seguros - Hoja Diaria</h1>
                    </div>
                </div>
                ...
                <table>
                    <thead>
                        <tr>
                            ${searchMode === 'range' ? '<th style="width: 85px;">Fecha Pago</th>' : ''}
                            <th>Seguro</th>
                            <th>Proforma #</th>
                            <th>Periodo</th>
                            <th style="text-align: right; width: 110px;">Monto</th>
                    ...
                    <tbody>
                        ${ingresosSeguros.length === 0 ? `
                            <tr><td colspan="${searchMode === 'range' ? 5 : 4}" style="text-align: center; font-style: italic; color: #64748b;">No hay registros</td></tr>
                        ` : ingresosSeguros.map(r => `
                            <tr>
                                ${searchMode === 'range' ? `<td>${formatDate(r.fecha_pago)}</td>` : ''}
    ...
    const renderContent = () => {
        ...
        switch (activeTab) {
            ...
            case 1: // Pagos Seguros
                return renderTableWithSummary([
                    ...(searchMode === 'range' ? [{ header: 'Fecha', accessor: (r: any) => formatDate(r.fecha_pago) }] : []),
    ...
    const currentTotals = calculateAllTotals(ingresos, egresos, pagosDoctores, pagosLaboratorios, pagosPedidos, pagosGastosFijos, ingresosSeguros);
