...
import type { Paciente, PacienteSeguro, Pago, Proforma, Agenda } from '../types';
...
const TABS_SEGURO: TabDef[] = [
    { id: 'ficha',       label: 'Ficha Médica',         icon: <Heart size={15} />,           path: 'ficha' },
    { id: 'odontograma', label: 'Odontograma',          icon: <Shield size={15} />,          path: 'odontograma' },
    { id: 'citas',       label: 'Citas',                icon: <Calendar size={15} />,       path: 'citas' },
    ...
    useEffect(() => {
        ...
        if (userString) {
            try {
                ...
                // CODEL uses 'permisos' as restricted modules
        ...
        const load = async () => {
            ...
            try {
                ...
                const [pacRes, agendaRes, pagosRes, proformasRes] = await Promise.allSettled([
                    api.get(tipo === 'particular' ? `${baseUrl}/${id}` : `${baseUrl}/${id}`), // Adjust if endpoint differs
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-blue-100 text-xs">
                                ...
                                {tipo === 'seguro' ? (
                                    <>
                                        <span>📱 {formatCelular((paciente as PacienteSeguro).celular)}</span>
                                        <span className="font-bold">
                                            🛡️ {(paciente as PacienteSeguro).es_trabajador ? 'Trabajador' : (paciente as PacienteSeguro).es_beneficiario ? 'Beneficiario' : '---'}
                                        </span>
                                    </>
                                ) : (
                                    <>
