...
interface Props {
    isOpen: boolean;
    onClose: () => void;
    onConfirm: (personalId: string | null) => void;
    vacaciones: Vacacion[];
    mode?: 'print' | 'export';
}

const VacacionesPrintModal: React.FC<Props> = ({ isOpen, onClose, onConfirm, vacaciones, mode = 'print' }) => {
    const [selectedPersonalId, setSelectedPersonalId] = useState('');
    const [personalList, setPersonalList] = useState<{ id: number, nombre: string }[]>([]);
        ...
        if (isOpen) {
            ...
            if (vacaciones.length > 0) {
                ...
                vacaciones.forEach(v => {
                    if (v.personal) {
                        if (!uniqueMap.has(v.personal.id)) {
                            uniqueMap.set(v.personal.id, {
                                id: v.personal.id,
                                nombre: formatFullName(v.personal)
                            });
                        }
                    }
                });
                const sorted = Array.from(uniqueMap.values()).sort((a, b) => a.nombre.localeCompare(b.nombre));
                setPersonalList(sorted);
            } else {
                setPersonalList([]);
            }
        }
    ...
    const handleConfirm = () => {
        onConfirm(selectedPersonalId || null);
        onClose();
    };
    ...
    const title = isExport ? 'Exportar Lista de Vacaciones' : 'Imprimir Lista de Vacaciones';
    const confirmText = isExport ? 'Exportar PDF' : 'Imprimir';
    const confirmClass = isExport
                            ...
                            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                ...
                                <div className="mt-2">
                                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                                        Seleccione el personal del cual desea {isExport ? 'exportar' : 'imprimir'} las vacaciones.
                                        Si deja la opción por defecto, se incluirán todos.
                                    </p>
...
export default VacacionesPrintModal;
