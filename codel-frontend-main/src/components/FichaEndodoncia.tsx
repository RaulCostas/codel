...
import type { FichaEndodoncia, EndodonciaPruebaVitalidad, EndodonciaControlTcr, EndodonciaMedicacion } from '../types';
import { Save, AlertCircle, Info, Stethoscope, Activity, Zap, FileText, Clipboard, Edit3, Plus, ArrowLeft, CheckSquare } from 'lucide-react';

interface FichaEndodonciaProps {
    pacienteId: number;
    proformaId: number;
    proformaNumero?: number;
}

const FichaEndodonciaForm: React.FC<FichaEndodonciaProps> = ({ pacienteId, proformaId, proformaNumero }) => {
    const [loading, setLoading] = useState(false);
    const [saving, setSaving] = useState(false);
    const [isEditMode, setIsEditMode] = useState(true);
    const [view, setView] = useState<'list' | 'form'>('list');
    const [formData, setFormData] = useState<FichaEndodoncia>({
        pacienteId,
        proformaId,
    ...
    const [registeredPieces, setRegisteredPieces] = useState<FichaEndodoncia[]>([]);
    ...
    const fetchRegisteredPieces = async () => {
        try {
            setLoading(true);
            const response = await api.get(`/ficha-endodoncia/proforma/${proformaId}/piezas`);
    ...
    const fetchFicha = async (pieza: string) => {
        try {
            setLoading(true);
            const response = await api.get(`/ficha-endodoncia/proforma/${proformaId}${pieza ? `?pieza=${pieza}` : ''}`);
            if (response.data) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            await api.post('/ficha-endodoncia', formData);
            Swal.fire({
                icon: 'success',
            ...
            // After saving, go back to list if this was a new piece
            setView('list');
        } catch (error) {
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                    <div className="p-6 bg-gradient-to-r from-red-700 to-rose-800 text-white flex items-center justify-between">
                        <div>
                            ...
                            <p className="mt-1 text-red-100/80 text-sm italic">Plan de Tratamiento #{proformaNumero || proformaId}</p>
                        </div>
                        <button
                    ...
                    <div className="p-8">
                        {loading ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-20 text-center">
                                ...
                                <p className="text-sm text-gray-400 mb-6">Este plan de tratamiento no tiene fichas de endodoncia aún.</p>
                                <button
                                    onClick={openNewPiece}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <div className="p-6 bg-gradient-to-r from-red-700 to-rose-800 text-white flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-bold flex items-center gap-3">
                            <span className="p-2 bg-white/20 rounded-lg"><Stethoscope size={24} /></span>
                            DIAGNÓSTICO Y PLAN DE TRATAMIENTO - ENDODONCIA
                        </h2>
                        <p className="mt-1 text-red-100/80 text-sm flex items-center gap-2 italic">
                            <Info size={14} />
                            Plan #{proformaNumero || proformaId}
                ...
                <div className="p-8 space-y-10">
                    {/* Pieza Dental - display only (navigation happens from list) */}
                    <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-6 border border-gray-100 dark:border-gray-700 flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-400 mt-0.5">Plan de Tratamiento #{proformaNumero || proformaId}</p>
                        </div>
                        {!formData.id && (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 border-t border-gray-100 dark:border-gray-700 pt-8">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-gray-50 dark:bg-gray-900/40 rounded-xl border border-gray-100 dark:border-gray-800 overflow-hidden">
                                ...
                                <div className="p-1">
                                    ...
                                    <PainRow label="POR CALOR" namePrefix="dolor_tipo_calor" />
                                    <PainRow label="POR FRÍO" namePrefix="dolor_tipo_frio" />
                                    <PainRow label="POR ACIDEZ" namePrefix="dolor_tipo_acidez" />
                                    <PainRow label="POR DULCE" namePrefix="dolor_tipo_dulce" />
                                    <PainRow label="POR MASTICACIÓN" namePrefix="dolor_tipo_masticacion" />
                                    <PainRow label="CONSTANTE" namePrefix="dolor_tipo_constante" />
                                    <PainRow label="DOLOR SORDO" namePrefix="dolor_tipo_sordo" />
                                    <PainRow label="DOLOR PALPITANTE" namePrefix="dolor_tipo_palpitante" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <label className="text-sm font-black text-gray-800 dark:text-gray-200 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800 pb-2">
                                4. Diagnóstico Pulpar
                            </label>
                            <div className="space-y-0.5">
                                <CheckboxItem label="Pulpa sana" name="pulpar_sana" checked={formData.pulpar_sana} />
                                <CheckboxItem label="Lesion de pulpa reversible" name="pulpar_reversible" checked={formData.pulpar_reversible} />
                                <CheckboxItem label="Pulpa irreversible sintomática" name="pulpar_irreversible_sintomatica" checked={formData.pulpar_irreversible_sintomatica} />
                                <CheckboxItem label="Pulpa irreversible asintomática" name="pulpar_irreversible_asintomatica" checked={formData.pulpar_irreversible_asintomatica} />
                                <CheckboxItem label="Necrosis pulpar" name="pulpar_necrosis" checked={formData.pulpar_necrosis} />
                                <CheckboxItem label="Pulpa previamente tratada" name="pulpar_previamente_tratada" checked={formData.pulpar_previamente_tratada} />
                                <CheckboxItem label="TCR indicado sin terminar" name="pulpar_tcr_sin_terminar" checked={formData.pulpar_tcr_sin_terminar} />
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-0.5">
                                <CheckboxItem label="Tejidos apicales saludables" name="peri_saludable" checked={formData.peri_saludable} />
                    ...
                    <div className="space-y-8 pt-8 border-t border-gray-100 dark:border-gray-700">
                                    ...
                                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                        {formData.pruebas_vitalidad?.map((row, idx) => (
                                            <tr key={idx} className="hover:bg-gray-50/50 dark:hover:bg-gray-750">
                                                ...
                                                <td className="p-2"><input type="text" disabled={!isEditMode} value={row.frio || ''} onChange={(e) => handleArrayChange('pruebas_vitalidad', idx, 'frio', e.target.value)} className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded bg-transparent text-gray-800 dark:text-white disabled:opacity-60" /></td>
                                                <td className="p-2"><input type="text" disabled={!isEditMode} value={row.calor || ''} onChange={(e) => handleArrayChange('pruebas_vitalidad', idx, 'calor', e.target.value)} className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded bg-transparent text-gray-800 dark:text-white disabled:opacity-60" /></td>
                                                <td className="p-2"><input type="text" disabled={!isEditMode} value={row.electrica || ''} onChange={(e) => handleArrayChange('pruebas_vitalidad', idx, 'electrica', e.target.value)} className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded bg-transparent text-gray-800 dark:text-white disabled:opacity-60" /></td>
                                                <td className="p-2"><input type="text" disabled={!isEditMode} value={row.percusion || ''} onChange={(e) => handleArrayChange('pruebas_vitalidad', idx, 'percusion', e.target.value)} className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded bg-transparent text-gray-800 dark:text-white disabled:opacity-60" /></td>
                                                <td className="p-2"><input type="text" disabled={!isEditMode} value={row.palpacion || ''} onChange={(e) => handleArrayChange('pruebas_vitalidad', idx, 'palpacion', e.target.value)} className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded bg-transparent text-gray-800 dark:text-white disabled:opacity-60" /></td>
                                                <td className="p-2"><input type="text" disabled={!isEditMode} value={row.estado || ''} onChange={(e) => handleArrayChange('pruebas_vitalidad', idx, 'estado', e.target.value)} className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded bg-transparent text-gray-800 dark:text-white disabled:opacity-60" /></td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                        ...
                        {/* Diagnóstico, Tratamiento y Re-Tratamiento */}
                                ...
                                <table className="w-full text-left text-xs bg-white dark:bg-gray-800">
                                    ...
                                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                        {formData.control_tcr?.map((row, idx) => (
                                            <tr key={idx} className="hover:bg-gray-50/50 dark:hover:bg-gray-750">
                                                <td className="p-2"><input type="text" disabled={!isEditMode} value={row.conductos_radiculares || ''} onChange={(e) => handleArrayChange('control_tcr', idx, 'conductos_radiculares', e.target.value)} className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded bg-transparent text-gray-800 dark:text-white disabled:opacity-60" /></td>
                                                <td className="p-2"><input type="text" disabled={!isEditMode} value={row.punto_referencia || ''} onChange={(e) => handleArrayChange('control_tcr', idx, 'punto_referencia', e.target.value)} className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded bg-transparent text-gray-800 dark:text-white disabled:opacity-60" /></td>
                                                <td className="p-2"><input type="text" disabled={!isEditMode} value={row.medida_provisional || ''} onChange={(e) => handleArrayChange('control_tcr', idx, 'medida_provisional', e.target.value)} className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded bg-transparent text-gray-800 dark:text-white disabled:opacity-60" /></td>
                                                <td className="p-2"><input type="text" disabled={!isEditMode} value={row.medida_trabajo || ''} onChange={(e) => handleArrayChange('control_tcr', idx, 'medida_trabajo', e.target.value)} className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded bg-transparent text-gray-800 dark:text-white disabled:opacity-60" /></td>
                                                <td className="p-2"><input type="text" disabled={!isEditMode} value={row.lima_inicial || ''} onChange={(e) => handleArrayChange('control_tcr', idx, 'lima_inicial', e.target.value)} className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded bg-transparent text-gray-800 dark:text-white disabled:opacity-60" /></td>
                                                <td className="p-2"><input type="text" disabled={!isEditMode} value={row.lima_maestra || ''} onChange={(e) => handleArrayChange('control_tcr', idx, 'lima_maestra', e.target.value)} className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded bg-transparent text-gray-800 dark:text-white disabled:opacity-60" /></td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                        ...
                        {/* Medicación Intraconducto & Observaciones */}
                                    ...
                                    <table className="w-full text-left text-xs bg-white dark:bg-gray-800">
                                        ...
                                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                            ...
                                            {formData.medicacion_intraconducto?.map((row, idx) => (
                                                <tr key={idx} className="hover:bg-gray-50/50 dark:hover:bg-gray-750">
                                                    <td className="p-2"><input type="date" disabled={!isEditMode} value={row.fecha || ''} onChange={(e) => handleArrayChange('medicacion_intraconducto', idx, 'fecha', e.target.value)} className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded bg-transparent text-gray-800 dark:text-white disabled:opacity-60 text-xs" /></td>
                                                    <td className="p-2"><input type="text" disabled={!isEditMode} value={row.medicacion || ''} onChange={(e) => handleArrayChange('medicacion_intraconducto', idx, 'medicacion', e.target.value)} className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded bg-transparent text-gray-800 dark:text-white disabled:opacity-60" placeholder="Ej: Hidróxido de calcio" /></td>
                                                    {isEditMode && (
...
export default FichaEndodonciaForm;
