...
const PagosLaboratoriosList: React.FC = () => {
    ...
    const filteredPagos = pagos.filter(pago => {
        ...
        const pacienteName = formatFullName(pago.trabajoLaboratorio?.paciente).toLowerCase();
                        ...
                        <tbody>
                            ${filtered.map((p: any, idx: number) => `
                                <tr>
                                    ...
                                    <td>${formatFullName(p.trabajoLaboratorio?.paciente)}</td>
    ...
    const handleExportPDF = async (laboratorioId: number | null, fechaInicio: string, fechaFinal: string) => {
        try {
            ...
            const tableRows = filtered.map((p: any) => [
                formatDate(p.fecha),
                formatFullName(p.trabajoLaboratorio?.paciente),
    ...
    const handleExportExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(filteredPagos.map(p => ({
            ...
            Paciente: formatFullName(p.trabajoLaboratorio?.paciente),
                            ...
                            return (
                                <tr key={pago.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td className="p-3 text-gray-700 dark:text-gray-300">
                                        {formatFullName(pago.trabajoLaboratorio?.paciente)}
                                    </td>
                                    <td className="p-3 text-gray-700 dark:text-gray-300">
                                    ...
                                    <td className="p-3 flex gap-2">
                                        <button
                                            ...
                                            title={isLocked ? 'Cerrar Caja' : canEditPayments ? 'Editar' : 'Sin permiso para editar pagos'}
                                        >
