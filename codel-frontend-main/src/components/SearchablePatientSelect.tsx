        ...
        const timer = setTimeout(async () => {
            ...
            try {
                ...
                if (allowType === 'both' || allowType === 'particular') {
                    promises.push(api.get(`/pacientes?search=${searchTerm}&limit=10`));
                } else {
                    promises.push(Promise.resolve({ data: { data: [] } }));
                }

                if (allowType === 'both' || allowType === 'seguro') {
                    promises.push(api.get(`/pacientes-seguro?search=${searchTerm}&limit=10`));
                } else {
                    promises.push(Promise.resolve({ data: { data: [] } }));
                }
                        ...
                        >
                            <div className="flex justify-between items-center">
                                ...
                                <span className={`text-[10px] px-2 py-0.5 rounded-full font-black uppercase ${
                                    ...
                                    {res.type === 'particular' ? 'PARTICULAR' : res.data.seguro?.nombre || 'SEGURO'}
                                </span>
                            </div>
                            <div className="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5">
                                CI: {res.data.ci || 'N/A'} {res.data.telefono_celular ? `| Cel: ${res.data.telefono_celular}` : ''}
                            </div>
                        </div>
                    ))}
                </div>
