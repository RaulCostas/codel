...
const PagosLaboratoriosForm: React.FC<PagosLaboratoriosFormProps> = ({ isOpen, onClose, id, preSelectedWorkId, onSaveSuccess }) => {
    ...
    }, [isOpen, id, preSelectedWorkId, isEdit]);
    ...
    const availablePatients = useMemo(() => {
        ...
        allUnpaidWorks.forEach(work => {
            ...
            if (work.laboratorio && work.laboratorio.id === selectedLabId && work.paciente) {
                patientsMap.set(work.paciente.id, formatFullName(work.paciente));
            }
        });
                ...
                <div className="w-full max-w-2xl h-full bg-white dark:bg-gray-800 shadow-2xl overflow-y-auto transform transition-transform animate-slide-in-right">
                    <div className="p-6">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-10 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl text-sm transition-all focus:ring-2 focus:ring-blue-500  bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                                        required
                                    />
                                </div>
                            </div>
                                    ...
                                    >
                                        ...
                                        {availableWorks.map((t) => (
                                            <option key={t.id} value={t.id}>
                                                {t.precioLaboratorio?.detalle || 'Trabajo'} - {t.fecha ? formatDate(t.fecha) : ''} - Bs. {formatNumber(t.total)}
                                            </option>
                                        ))}
                                    </select>
                                </div>
                                ...
                                {moneda === 'Dólares' && (
                                    <div>
                                        ...
                                        <input
                                            ...
                                            placeholder="Ej: 6.96"
                                            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl text-sm transition-all focus:ring-2 focus:ring-blue-500  bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                                        />
                                    </div>
                                )}
                            </div>
                            ...
                            {selectedWork && (
                                <div className="bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl p-4 text-right">
