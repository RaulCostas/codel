    ...
    const exportToExcel = async () => {
        try {
            ...
            const data = allPagos.map((p: any) => ({
                'Doctor': p.doctor ? formatFullName(p.doctor) : 'N/A',
    ...
    const exportToPDF = async () => {
        try {
            ...
            const tableData = allPagos.map((p: any) => [
                p.doctor ? formatFullName(p.doctor) : 'N/A',
                formatDate(p.fecha),
    ...
    const handlePrint = async (pagoSummary: any) => {
        try {
            ...
            const tableRows = pago.detalles?.map((d: any) => {
                ...
                const paciente = hc?.paciente ? formatFullName(hc.paciente) : 'Desconocido';
                    ...
                    <div class="info-box">
                        <div class="info-row">
                            <div class="info-col">
                                ...
                                <span class="info-value">${pago.doctor ? formatFullName(pago.doctor) : 'N/A'}</span>
                            </div>
                            <div class="info-col">
                        ...
                        {Array.isArray(pagos) && pagos.map((pago: any, index: number) => {
                            ...
                            return (
                                <tr key={pago.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                    ...
                                    <td className="p-4 font-medium text-gray-800 dark:text-white">{pago.doctor ? formatFullName(pago.doctor) : 'N/A'}</td>
                                    ...
                                    <td className="p-4 text-right font-bold text-green-600 dark:text-green-400">
                                        {['Dólares', '$us', 'Sus', 'USD'].includes(pago.moneda) ? '$us' : 'Bs'} {formatNumber(pago.total)}
                                    </td>
                                        ...
                                        <div className="flex justify-center items-center gap-2">
                                            ...
                                            {canEditPayments && !isLocked ? (
                                                ...
                                                <span
                                                    ...
                                                    title={isLocked ? 'Cerrar Caja' : 'Sin permiso para editar pagos'}
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="currentColor">
                                                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                                    </svg>
                                            ...
                                            {filteredCancelledTreatments.length > 0 ? (
                                                filteredCancelledTreatments.map((item: any) => (
                                                    <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                                        ...
                                                        <td className="px-4 py-4 whitespace-nowrap text-xs text-gray-600 dark:text-gray-300">
                                                            {item.paciente ? formatFullName(item.paciente) : '-'}
                                                        </td>
                                                        <td className="px-4 py-4 whitespace-nowrap text-xs text-gray-600 dark:text-gray-300">
