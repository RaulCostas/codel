            ...
            try {
                ...
                if (data) {
                    setFormData(prev => ({
                        ...
                        monto: data.monto_pendiente ? data.monto_pendiente.toString() : ''
                    }));
                    if (data.clinica) {
                        setClinicaNombre(data.clinica.nombre);
                        setClinicaLogo(data.clinica.logo || '');
                    }
                }
            } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await Swal.fire({
                ...
                text: `Gracias por registrar su pago. Puedes pasar a recepción.`,
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col items-center py-10 px-4">
            <div className="w-full max-w-[600px] bg-white rounded-xl shadow-lg p-6 md:p-10">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl font-extrabold text-blue-700 tracking-tight">CODEL</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <p className="text-sm font-medium text-blue-600 bg-blue-50 px-3 py-1 rounded-full border border-blue-100 shadow-sm">
                        CODEL
                    </p>
                    <div>
                    ...
                    <div className="pt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <button
                            type="submit"
                            disabled={loading}
                            className={`w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 px-6 rounded-lg text-xl transition-all shadow-lg flex justify-center items-center ${loading ? 'opacity-70 cursor-not-allowed' : 'hover:-translate-y-1'}`}
                        >
                        ...
                        <button
                            ...
                            disabled={loading}
                            className={`w-full bg-amber-50 border border-amber-200 text-amber-700 font-bold py-4 px-6 rounded-lg text-xl transition-all shadow-sm flex justify-center items-center gap-2 ${loading ? 'opacity-70 cursor-not-allowed' : 'hover:bg-amber-100 hover:-translate-y-0.5'}`}
                        >
