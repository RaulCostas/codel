...
const AgendaView: React.FC = () => {
    ...
    const fetchTurno = async () => {
        try {
            const response = await api.get(`/turnos-personal/fecha/${currentDate}`);
            setCurrentTurno(response.data || null);
        } catch (error) {
    ...
    const fetchPersonal = async () => {
        try {
            ...
            setPersonalList(response.data.data || response.data || []);
        } catch (error) {
            console.error('Error fetching personal:', error);
        }
    };

    const handleSaveTurno = async (personalId: number) => {
        try {
            const userStr = localStorage.getItem('user');
            const user = userStr ? JSON.parse(userStr) : null;
            const usuarioId = user?.id;

            await api.post('/turnos-personal', { 
                fecha: currentDate, 
                personalId,
                usuarioId
            });
    ...
    const handleDeleteTurno = async () => {
        ...
        try {
            await api.delete(`/turnos-personal/${currentTurno.id}`);
            fetchTurno();
            Swal.fire({
                icon: 'info',
                title: 'Turno quitado',
            ...
            Swal.fire('Error', 'No se pudo quitar el turno', 'error');
        }
    };
                ...
                <div className="grid grid-cols-7 flex-1 overflow-y-auto">
                    {days.map((dayObj, index) => {
                        ...
                        return (
                            <div 
                                key={index} 
                                className={`min-h-[80px] sm:min-h-[120px] p-1 border-b border-r border-gray-200 dark:border-gray-700 transition-colors ${dayObj.currentMonth ? 'bg-white dark:bg-gray-800 hover:bg-blue-50/30 dark:hover:bg-gray-750' : 'bg-gray-50 dark:bg-gray-900/50 text-gray-300 dark:text-gray-600'} ${dayObj.date ? 'cursor-pointer' : ''}`}
                                onClick={() => {
                                    if (dayObj.date) {
                                ...
                                <div className="space-y-1">
                                    {dayObj.appointments?.slice(0, 4).map((app, appIndex) => (
                                        <div 
                                            ...
                                            style={{ backgroundColor: (app.pacienteSeguro?.seguro?.color || (app.paciente ? '#6366f1' : getStatusColor(app.estado))) }}
                                        >
                                            {app.hora.substring(0, 5)} {(app.paciente || app.pacienteSeguro) ? formatFullName(app.paciente || app.pacienteSeguro) : (app.tratamiento || 'Bloqueo')}
                                        </div>
                                    ))}
        ...
        <div className="flex flex-col h-[85vh] p-2 md:p-5">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-2 md:mb-6 no-print gap-2 md:gap-4">
                ...
                <div className="flex gap-2 flex-wrap justify-center md:justify-end items-center">
                    ...
                    {viewMode === 'day' && (
                        <div className="flex items-center gap-2 bg-blue-50 dark:bg-blue-900/20 px-3 py-1.5 rounded-lg border border-blue-100 dark:border-blue-800 shadow-sm transition-all hover:shadow-md cursor-pointer group mr-2"
                            onClick={() => setShowTurnoModal(true)}
                        >
                            <div className="p-1 bg-blue-100 dark:bg-blue-800 rounded-full text-blue-600 dark:text-blue-300 group-hover:scale-110 transition-transform">
                            ...
                            <div className="flex flex-col">
                                <span className="text-[9px] font-bold text-blue-500 dark:text-blue-400 uppercase tracking-wider leading-none mb-0.5">Personal de Turno</span>
                                <span className="text-xs font-black text-blue-800 dark:text-blue-100 leading-none">
                                    {currentTurno ? formatFullName(currentTurno.personal) : 'Sin asignar'}
                                </span>
                            </div>
                        </div>
                    )}
            ...
            <div className="flex flex-col md:flex-row-reverse gap-5 flex-1 overflow-hidden">
                            ...
                            <table className="min-w-[800px] w-full border-collapse table-fixed">
                                <thead className="sticky top-0 bg-gray-50 dark:bg-gray-700 z-10 shadow-sm">
                                    <tr>
                                        ...
                                        {[1, 2, 3, 4].map(num => (
                                            <th key={num} className="border border-gray-300 dark:border-gray-600 p-2 text-center font-bold text-gray-700 dark:text-gray-200 text-[10px] sm:text-xs">CONSULTORIO #{num}</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody>
                                    {timeSlots.map(time => (
                                        <tr key={time}>
                                            <td className="border border-gray-300 dark:border-gray-600 p-1 text-center bg-gray-50 dark:bg-gray-750 font-bold text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs align-middle">{time}</td>
                                            {[1, 2, 3, 4].map(consultorio => {
                                                ...
                                                const bgColor = appointment
                                                    ? (appointment.pacienteSeguro?.seguro?.color || (appointment.paciente ? '#6366f1' : getStatusColor(appointment.estado)))
                                                    : undefined;
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        {appointment && (
                                                            <div className="h-full flex flex-col justify-center text-[10px] overflow-hidden text-white drop-shadow-md pl-3 pr-1 py-1 rounded-sm relative"
                                                            >
                                                                {/* Status Bar - More prominent */}
                                                                <div 
                                                                    className="absolute top-0 left-0 bottom-0 w-[8px] shadow-[1px_0_3px_rgba(0,0,0,0.3)] z-20"
                                                                    style={{ backgroundColor: getStatusColor(appointment.estado) }}
                                                                    title={`Estado: ${appointment.estado}`}
                                                                />
                                                                ...
                                                                <div className="font-bold truncate pr-6 ml-1">
                                                                    {(appointment.paciente || appointment.pacienteSeguro) ? (
                                                                        <span 
                                                                            className="hover:underline cursor-pointer transition-all hover:text-white/80"
                                                                            onClick={(e) => {
                                                                                e.stopPropagation();
                                                                                const id = appointment.pacienteId || appointment.pacienteSeguroId;
                                                                                const type = appointment.pacienteId ? 'pacientes' : 'pacientes-seguro';
                                                                                // Navigate to the patient's profile
                                                                                navigate(`/${type}/${id}/ficha`);
                                                                            }}
                                                                        >
                                                                            {formatFullName(appointment.paciente || appointment.pacienteSeguro)}
                                                                        </span>
                                                                    ) : (
                                                                ...
                                                                {(appointment.paciente || appointment.pacienteSeguro) && appointment.tratamiento && (
                                                                ...
                                                                <div className="text-[9px] mt-0.5 font-bold uppercase opacity-80 flex items-center justify-between gap-1 w-full">
                                                                    ...
                                                                    {(appointment.paciente || appointment.pacienteSeguro) && appointment.estado === 'agendado' && (
                                                                        <button
                                        ...
                                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                            {patientHistory.map((cita) => (
                                                <tr key={cita.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                                    ...
                                                    <td className="p-3">
                                                        <span className="px-2 py-1 rounded-full text-xs font-bold text-white shadow-sm" style={{ backgroundColor: cita.pacienteSeguro?.seguro?.color || cita.paciente?.seguro?.color || getStatusColor(cita.estado) }}>
                ...
                {showTurnoModal && (
                    <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] backdrop-blur-sm">
                        <div className="bg-white dark:bg-gray-800 w-[400px] max-w-[90%] rounded-xl shadow-2xl overflow-hidden border border-gray-200 dark:border-gray-700">
                            <div className="p-5 border-b border-gray-200 dark:border-gray-700 bg-blue-600 text-white flex justify-between items-center">
                                <h3 className="text-lg font-bold m-0 flex items-center gap-2">
                                    ...
                                    Asignar Personal de Turno
                                </h3>
                            </div>
                            <div className="p-6">
                                <p className="text-sm text-gray-500 dark:text-gray-400 mb-4 font-medium">
                                    Seleccione el personal responsable para el día: <span className="text-blue-600 dark:text-blue-400 font-bold">{formatDate(currentDate)}</span>
                                </p>
                                <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                                    {personalList.map(p => (
                                        <button
                                            key={p.id}
                                            onClick={() => handleSaveTurno(p.id)}
                                            className={`w-full text-left p-3 rounded-lg border transition-all flex items-center justify-between group ${currentTurno?.personalId === p.id 
                                                ? 'bg-blue-50 border-blue-500 dark:bg-blue-900/30 dark:border-blue-400' 
                                                : 'bg-gray-50 border-gray-200 dark:bg-gray-700/50 dark:border-gray-600 hover:border-blue-400 dark:hover:border-blue-500 hover:bg-white dark:hover:bg-gray-700'}`}
                                        >
                                            <div className="flex flex-col">
                                                <span className={`font-bold text-sm ${currentTurno?.personalId === p.id ? 'text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-200'}`}>
                                                    {formatFullName(p)}
                                                </span>
                                                <span className="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-semibold">
                                                    {p.personalTipo?.nombre || 'Personal'}
                                                </span>
                                            </div>
                                            {currentTurno?.personalId === p.id && (
                                ...
                                <div className="p-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center">
                                    {currentTurno && (
                                        <button 
                                            ...
                                            className="bg-red-50 hover:bg-red-100 text-red-600 dark:bg-red-900/20 dark:hover:bg-red-900/30 dark:text-red-400 text-xs font-bold py-1.5 px-3 rounded-lg flex items-center gap-1 transition-all active:scale-95 border border-red-200 dark:border-red-800"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                    ...
                                    <button 
                                        ...
                                        className="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded-lg flex items-center gap-2 transform hover:-translate-y-0.5 transition-all active:scale-95 shadow-md text-sm ml-auto"
                                    >
