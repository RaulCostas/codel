...
const ArancelList: React.FC<ArancelListProps> = ({ tipo }) => {
    ...
    useEffect(() => {
        fetchAranceles();
        fetchEspecialidades();
    }, [currentPage, debouncedSearchTerm, tipo]);
    ...
    const fetchAranceles = async () => {
        try {
            ...
            const endpoint = tipo === 'seguro' ? '/arancel-seguro' : '/arancel';
    ...
    const handleDelete = async (id: number) => {
        ...
        if (result.isConfirmed) {
            try {
                const endpoint = tipo === 'seguro' ? `/arancel-seguro/${id}` : `/arancel/${id}`;
                await api.patch(endpoint, { estado: 'inactivo' });
                await Swal.fire({
                    icon: 'success',
    ...
    const handleReactivate = async (id: number) => {
        ...
        if (result.isConfirmed) {
            try {
                const endpoint = tipo === 'seguro' ? `/arancel-seguro/${id}` : `/arancel/${id}`;
                await api.patch(endpoint, { estado: 'activo' });
                await Swal.fire({
                    icon: 'success',
    ...
    const exportToPDF = async () => {
        try {
            ...
            // Prepare table data
            const headers = ['#', 'Descripción', 'Especialidad', 'Precio', 'Estado'];
            if (tipo === 'seguro') headers.splice(1, 0, 'Código');
            ...
            const tableData = aranceles.map((a, index) => {
                ...
                if (tipo === 'seguro') row.splice(1, 0, a.codigo || '');
                return row;
            });
    ...
    const handlePrint = async () => {
        try {
            // Fetch ALL records for printing with clinic & type filter
    ...
    return (
        <div className="content-card bg-white dark:bg-gray-800 shadow-xl rounded-2xl p-8 transition-all duration-300">
            {/* Header */}
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 no-print gap-4">
                <div className="flex flex-col">
                    <h2 className="text-3xl font-bold text-gray-800 dark:text-white flex items-center gap-3">
                        <ClipboardList className="text-blue-600" size={32} />
                        Lista de Aranceles {tipo === 'particular' ? '(Particulares)' : tipo === 'seguro' ? '(Seguro)' : ''}
                    </h2>
                ...
                <div className="flex gap-2 flex-wrap justify-center md:justify-end items-center">
                    ...
                    <div className="flex gap-2 items-center">
                        <button
                            ...
                            className="bg-[#28a745] hover:bg-green-600 text-white font-semibold py-2 px-4 rounded-xl flex items-center justify-center shadow-md transition-all transform hover:-translate-y-0.5 gap-2"
                        ...
                        <button
                            onClick={exportToPDF}
                            className="bg-[#dc3545] hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-xl flex items-center justify-center shadow-md transition-all transform hover:-translate-y-0.5 gap-2"
                        ...
                        <button
                            onClick={handlePrint}
                            className="bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-4 rounded-xl flex items-center justify-center shadow-md transition-all transform hover:-translate-y-0.5 gap-2"
                            title="Imprimir"
                        >
                            <Printer size={18} />
                ...
                <div className="flex gap-2 w-full md:max-w-md">
                    <div className="relative flex-grow">
                        <input
                            ...
                            className="flex-grow pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-blue-500 outline-none transition-all duration-300"
                        />
                ...
                <div className="flex gap-4 items-center">
                    <div className="flex gap-2">
                        <button
                            ...
                            className="bg-[#1abc9c] hover:bg-teal-600 text-white font-semibold py-2 px-4 rounded-xl flex items-center gap-2 shadow-md transition-all transform hover:-translate-y-0.5"
                        >
            ...
            {showUpdateModal && (
                <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-50 transition-opacity">
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl w-full max-w-md shadow-xl transform transition-all">
                        ...
                        <form onSubmit={handleUpdatePrices}>
                            ...
                            <div className="flex justify-end gap-2">
                                ...
                                <button
                                    type="submit"
                                    className="px-4 py-2 bg-[#1abc9c] text-white rounded hover:bg-teal-600 font-bold transition-all"
                                >
                                    Aceptar
                                </button>
                            </div>
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {aranceles.map((a, index) => (
                            <tr key={a.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all">
                                ...
                                <td className="p-3 text-gray-800 dark:text-gray-300 font-bold">{formatCurrency(a.precio)}</td>
                                <td className="p-3">
                                ...
                                <td className="p-3 flex gap-2">
                                    ...
                                    {a.estado === 'activo' ? (
                                        <button
                                            ...
                                            className="bg-[#dc3545] hover:bg-red-700 text-white p-2 rounded-xl shadow-md transition-all transform hover:-translate-y-0.5 flex items-center justify-center"
                                            title="Dar de baja"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="currentColor">
                                        ...
                                        <button
                                            ...
                                            className="bg-green-600 hover:bg-green-700 text-white p-2 rounded-xl shadow-md transition-all transform hover:-translate-y-0.5 flex items-center justify-center"
                                            title="Reactivar"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="currentColor">
